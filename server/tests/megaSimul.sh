...
url='https://api.villagetech.in:8090/'
...
while [ $user_iter -le $user_count ]
  do
    echo "INSERT INTO user(an, pn, auth, tid, pid, age, gdr, name, hs, dl) VALUES('$user_iter', '97000000$user_iter', 'auth$user_iter', -1, FLOOR(RAND()*(10-5+1)+1),  25, 'MALE', 'John $user_iter',  'UK', 'UK-1234567890$user_iter');"
    ((user_iter++))
...
while [ $driver_iter -le $driver_count ]
  do
    echo "INSERT INTO driver (an, pn, auth, dl, mode, tid, pid, name, age, gdr, hs) VALUES('$driver_iter', '98000000$driver_iter', 'dauth$driver_iter', 'UK01234567891$driver_iter', 'OF', -1,FLOOR(RAND()*(10-5+1)+1), 'driver0$driver_iter', 30, 'm', 'uk');"
    ((driver_iter++))
  done | mysql --user=$ZP_DB_USER --password=$ZP_DB_PASSWD $ZP_DB_NAME
echo "created $driver_count drivers."
...
while [ $vehicle_iter -le $vehicle_count ]
  do
    echo "INSERT INTO vehicle(regn, dist, hrs, pid, vtype, an, tid) VALUES('reg$vehicle_iter', '0', '0', FLOOR( 1 + RAND() * 7 ),  FLOOR( 1 + RAND() * 3 ), '20$vehicle_iter',  '-1');"
    ((vehicle_iter++))
  done | mysql --user=$ZP_DB_USER --password=$ZP_DB_PASSWD $ZP_DB_NAME
echo "created $vehicle_count vehicles."
